...
RSpec.describe Conductors::Spin do
  subject { Conductors::Spin.new }
  describe '#run' do
    let(:payload) {
      '{
    ...
    let(:web_service) { instance_double(WebServices::Spin,
                                        get_data: payload_hash
    ) }

    let(:brand) { instance_double(Brand, name: 'Doggie') }

    before do
      allow(WebServices::Spin).to receive(:new).
  ...
  it 'assigns itself a name' do
    expect(subject.name).to eq('Spin')
  end
end