...
RSpec.describe Conductors::Lime do
  subject { Conductors::Lime.new }
  describe '#run' do
    let(:payload) {
      '{
    ...
    let(:web_service) { instance_double(WebServices::Lime,
    ...
    before do
      allow(WebServices::Lime).to receive(:new).
  ...
  it 'assigns itself a name' do
    expect(subject.name).to eq('Lime')
  end
end