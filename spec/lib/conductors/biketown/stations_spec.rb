...
describe Conductors::Biketown::Stations do
  subject { Conductors::Biketown::Stations.new }
  ...
  describe '#run' do
    ...
    let(:web_service) { instance_double(WebServices::Biketown::Stations,
                                        get_data: payload_hash
    ) }

    let(:brand) { instance_double(Brand, name: 'Fox' )}

    before do
      allow(WebServices::Biketown::Stations).to receive(:new).
        and_return(web_service)
      allow(Brand).to receive(:find_by).
        and_return(brand)
      allow(ShareStation).to receive(:for_hash).
    ...
    it 'creates models for the data' do
      expect(ShareStation).to receive(:for_hash).with(payload_hash, brand)
      subject.run
    end
  end
